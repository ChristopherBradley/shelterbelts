...
Most source files have a corresponding Jupyter Notebook to demonstrate usage and provide a familiar environment for new users to play around with the outputs. The Jupyter Notebooks are initially saved as .py files during development using the Jupytext library to avoid overloading the git history with slight changes to large image files. These are committed as a .ipynb file once finalised so the outputs are easily viewable from within the github interface.

Most source files also have a corresponding test file for common scenarios to quickly check that new changes do not break existing parts of the codebase. We have prioritised testing common scenarios and past known failure mechanisms over test completeness to better support fast iterative development.
...
The Shelterbelts Earth Engine app allows users to toggle the global canopy height and worldcover layers, and to use these data sources from within the repository to apply the tree and shelter categorisation at any location globally. Functions are provided for creating gully rasters from global elevation data and road rasters from Open Street Maps, used as inputs to the pipeline. Functions are not currently provided for downloading global wind direction data, but these do exist (e.g. ERA-5), or the pipeline can be run without wind data by using the percent cover or any direction wind methods.

One future research direction is to address uncertainty about context-dependent benefits of shelterbelts that may hinder adoption. The software can be used to map variability in benefits at large spatial scales by providing gridded estimates of the degree of shelter provided in different categories, which can then be aligned with satellite derived productivity estimates. These kinds of analyses could complement the growing policy and public funding support to plant trees on farms for benefits including carbon drawdown, biodiversity, and agricultural resilience.

# Acknowledgements

This work used computational resources provided by the National Computational Infrastructure (NCI Australia). We thank Geoscience Australia, Digital Earth Australia, and the developers of the open-source software used in this work. The authors would like to thank Nicolas Pucino for providing the initial training data for predicting the 10m binary tree data.

# AI Usage Disclosure

Various versions of Claude Code were used to assist with software development, particularly debugging and proofreading. The authors take full responsibility for the software and manuscript.
