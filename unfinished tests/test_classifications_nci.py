...
filename_scaler = os.path.join(outdir, f'{stub}_scaler.pkl') # Generated by neural_network
filename_model = os.path.join(outdir, f'{stub}_nn.keras') # Generated by neural_network

def test_basic():
    """Basic tests for each of the files"""
    download_ds2(sample_tif, start_date="2020-01-01", end_date="2021-01-01", outdir=outdir)
    tile_csv(sentinel_tile, tree_file=sample_tif, outdir=outdir, radius=4, spacing=1)  # I want to implement another way of doing this using all 12 months instead of spatial and temporal mean and standard deviation, and with geometric media
    ...
    tif_prediction(sentinel_tile, outdir, filename_model, filename_scaler)
...
def test_neural_network_unknown():
    """More comprehensive tests for using the model to predict unseen locations"""
    # Using the same sentinel_tile as before to avoid downloading multiple (since it takes 5 mins when running locally), but this could work on any sentinel tile now
    tif_prediction(sentinel_tile, outdir, filename_model, filename_scaler)
