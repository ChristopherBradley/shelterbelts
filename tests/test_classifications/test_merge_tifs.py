...
def test_merge_tifs_basic():
    ...
    outpath = os.path.join(os.path.dirname(quartered_tifs_dir), 'quartered_linear_tifs_merged.tif')
...
def test_merge_tifs_colormap():
    ...
    merge_tifs(quartered_tifs_dir, tmpdir='tmpdir', suffix='.tif', dont_reproject=True, dedup=True)
    outpath = os.path.join(os.path.dirname(quartered_tifs_dir), 'quartered_linear_tifs_merged.tif')
