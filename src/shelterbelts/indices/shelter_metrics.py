...
def majority_filter_override(data, footprint, classes_that_override, classes_to_override):
    ...
    def selective_majority(values, centre_val):
        ...
        if not override_vals:
            return centre_val  # keep original
    ...
    # Pad the array so centre pixel is accessible in each window
...
def skeleton_stats(assigned_labels, min_patch_size=20, save_labels=True):
    ...
    for i, prop in enumerate(props):
        ...
        # coordinates of the two perpendicular neighbours
...
def patch_metrics(buffer_data, outdir=".", stub="TEST", plot=True, save_csv=True, save_tif=True, save_labels=True, 
    ...
    ----------
        ...
        outdir : str, optional
            Output directory for saving results. Default is '.'.
        stub : str, optional
...
def parse_arguments():
    ...
    parser.add_argument('--min_shelterbelt_length', default=15, type=int, help='Minimum skeleton length (in pixels) to classify a cluster as linear')
    parser.add_argument('--max_shelterbelt_width', default=6, type=int, help='Maximum skeleton width (in pixels) to classify a cluster as linear')
    ...
    parser.add_argument('--crop_pixels', default=None, type=int, help='Number of pixels to crop from each edge of the output')
