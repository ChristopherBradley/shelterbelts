...
def shelter_categories(category_data, wind_data=None, height_tif=None, outdir='.', stub='TEST', wind_method='WINDWARD', wind_threshold=20, distance_threshold=20, density_threshold=5, savetif=True, plot=True, crop_pixels=None):
    ...
    wind_method : str, optional
        Either 'WINDWARD', 'MOST_COMMON', 'MAX', 'HAPPENED', 'ANY', or 'MULTI_LAYER'. Default is 'WINDWARD'.
        ...
        MULTI_LAYER saves an 8-band GeoTIFF (bands in clockwise order: N, NE, E, SE, S, SW, W, NW). Does not require wind_data.
    ...
    if wind_method == 'MULTI_LAYER':
        ...
        distance_rasters = [
            compute_distance_to_tree_TH(shelter_heights, d, distance_threshold)
                .fillna(0).clip(0, 255).astype('uint8')
            for d in directions
        ]
        da_multi = xr.concat(distance_rasters, dim='direction').assign_coords(direction=directions)
        da_multi = da_multi.rio.write_crs(da_categories.rio.crs)
...
def parse_arguments():
    ...
    parser.add_argument('--wind_method', default='WINDWARD', help="Either 'WINDWARD', 'MOST_COMMON', 'MAX', 'HAPPENED', 'ANY', or 'MULTI_LAYER'")
