...
from scipy.ndimage import convolve, maximum_filter, label, center_of_mass
...
def crowns_to_gpkg(chm_tif, outdir, stub, height_threshold=2, max_crown_m=10):
    ...
    growing, then converts the labelled raster to a GeoPackage with per-crown
    height statistics.
    ...
    chm_raw = np.nan_to_num(chm_raw, nan=0.0)
    chm_raw = np.clip(chm_raw, 0, None)

    kernel = np.ones((3, 3)) / 9.0
    chm = convolve(chm_raw, kernel).astype(np.float32)

    ws_pixels = max(1, int(5 / resolution))
    chm_max = maximum_filter(chm, footprint=np.ones((ws_pixels, ws_pixels)))
    tree_maxima = (chm == chm_max) & (chm > height_threshold)
    tree_markers, num_trees = label(tree_maxima)

    if num_trees == 0:
        print(f"No trees detected in {chm_tif}")
        return None

    yx = np.array(center_of_mass(chm, tree_markers, range(1, num_trees + 1)), dtype=np.float32) + 0.5
    seeds = np.array([yx[:, 1], yx[:, 0]], dtype=np.int32)
    ...
    mask = crowns > 0
    geoms, ids = [], []
    for geom_dict, val in rio_shapes(crowns, mask=mask, transform=transform):
