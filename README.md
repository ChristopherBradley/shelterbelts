...
The manuscript source is in `JOSS_Paper` along with figures generated by draw.io. To merge the main text with references and figures into a pdf:

`cd JOSS_Paper`  
`pandoc paper.md --bibliography=paper.bib --citeproc --pdf-engine=tectonic -o paper.pdf`

`pandoc` and `tectonic` are included in `environment.yml` and will be available after `conda env create -f environment.yml`. Tectonic downloads LaTeX packages automatically on first run (requires internet).
